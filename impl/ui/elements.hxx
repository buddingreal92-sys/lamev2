...
namespace ui {
    ...
    inline void draw_glass_card( ImDrawList* dl, const ImVec2& p0, const ImVec2& p1, float = 0.0f, ImU32 = 0 ) {
        dl->AddRectFilled( p0, p1, theme::panel_bg(), theme::panel_rounding );
        dl->AddRect( p0, p1, theme::border(), theme::panel_rounding, 0, 1.0f );
        dl->AddLine( ImVec2( p0.x + 1.f, p0.y + 24.f ), ImVec2( p1.x - 1.f, p0.y + 24.f ),
    ...
    inline bool checkbox( const char* label, bool* v, const char* bind_id = nullptr, const char* default_bind = nullptr, float col_w = 226.0f ) {
        ...
        dl->AddRectFilled( sp0, sp1, *v ? theme::accent() : IM_COL32( 13, 13, 13, 255 ) );
        dl->AddRect( sp0, sp1, *v ? theme::accent() : theme::border_bright(), 0.f, 0, 1.0f );

        const float tly = snap_text_y( pos.y + ( row_h - ImGui::GetTextLineHeight( ) ) * 0.5f );
        dl->AddText( ImVec2( pos.x + box_size + 8.f, tly ), hov ? theme::text_bright() : theme::text(), label );
    ...
    inline bool slider_float( const char* label, float* v, float vmin, float vmax, const char* suffix = "", const char* fmt = "%.0f", float col_w = 226.0f ) {
        ...
        const float total_h = lbl_h + 5.0f + track_h + 4.0f;
        ...
        dl->AddRectFilled( tp0, tp1, hov ? IM_COL32( 40, 40, 40, 255 ) : theme::track_bg() );

        if ( t > 0.001f ) {
            const ImVec2 fe = ImVec2( tp0.x + t * avail_w, tp1.y );
            dl->AddRectFilled( tp0, fe, theme::track_fill() );
        }
        const float marker_x = tp0.x + t * avail_w;
        dl->AddRectFilled( ImVec2( marker_x - 1.f, tp0.y - 2.f ), ImVec2( marker_x + 1.f, tp1.y + 2.f ),
            ( held || hov ) ? theme::text_bright() : theme::handle() );
    ...
    inline bool dropdown( const char* label, int* sel, const char** items, int count, float col_w = 226.0f ) {
        ...
        dl->AddRectFilled( pos, ImVec2( pos.x + w, pos.y + h ), hov ? theme::control_hover() : theme::control_bg() );
        dl->AddRect( pos, ImVec2( pos.x + w, pos.y + h ), is_open ? theme::accent_dim() : theme::border(), 0.f, 0, 1.0f );
    ...
    inline void render_open_dropdown( ) {
        ...
        fdl->AddRectFilled( p0, p1, theme::control_bg() );
        fdl->AddRect( p0, p1, theme::border_bright(), 0.f, 0, 1.0f );
        ...
        for ( int i = 0; i < _dd::count; i++ ) {
            ...
            if ( hov_i ) {
                fdl->AddRectFilled( ip0, ip1, theme::tab_hover_bg() );
                fdl->AddRectFilled( ImVec2( ip0.x, ip0.y ), ImVec2( ip0.x + 2, ip1.y ), theme::accent() );
            }
    ...
    inline bool button( const char* label, float w, float h = 22.0f ) {
        ...
        dl->AddRect( pos, p1, border, 0.f, 0, 1.0f );

        const ImVec2 ts = ImGui::CalcTextSize( label );
        const float tx = pos.x + ( w - ts.x ) * 0.5f;
        const float ty = snap_text_y( pos.y + ( h - ts.y ) * 0.5f );
    ...
    inline bool text_input( const char* id, char* buf, int buf_size, float w, float h = 22.0f ) {
        ...
        dl->AddRectFilled( pos, p1, hov ? theme::control_hover() : theme::control_bg() );
        const ImU32 bord = focused ? theme::accent() : ( hov ? theme::border_bright() : theme::border() );
        dl->AddRect( pos, p1, bord, 0.f, 0, 1.0f );
