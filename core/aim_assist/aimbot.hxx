...
namespace aim_assist {
    ...
    public:
        ...
        void update( const osu::game_snapshot_t& game, const osu::beatmap_data_t& map ) {
            ...
            const float local_difficulty = compute_local_difficulty(
                snap->targets, osu_hit_radius, game.speed_mult );
            const adaptive_parameters_t adaptive = update_adaptive_parameters( local_difficulty );
    ...
    private:
        ...
        static float compute_local_difficulty(
            ...
            return assist::saturate(
                0.55f * weighted_mean + 0.25f * peak + 0.20f * consecutive_score );
        }

        adaptive_parameters_t update_adaptive_parameters( float raw_difficulty ) {
            std::lock_guard<std::mutex> lock( m_apply_mutex );
            ...
            const float response = target > m_adaptive_difficulty ? 0.22f : 0.85f;
            ...
            const float centered_difficulty = m_adaptive_difficulty - 0.35f;

            adaptive_parameters_t result{};
            result.difficulty = m_adaptive_difficulty;
            result.strength = std::clamp(
                assist_strength * ( 1.f + influence * 0.35f * centered_difficulty ),
                5.f, 100.f );
            result.smoothing_ms = std::clamp(
                smoothing_ms * ( 1.f - influence * 0.45f * centered_difficulty ),
                35.f, 180.f );
            result.max_correction = std::clamp(
                max_correction * ( 1.f + influence * 0.35f * centered_difficulty ),
                0.10f, 1.25f );
            result.anticipation_ms = std::clamp(
                520.f + influence * 140.f * centered_difficulty,
                440.f, 640.f );
